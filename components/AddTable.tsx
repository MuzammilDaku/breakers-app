...
const AddTableComp: React.FC = () => {
  ...
  const tableStr = Array.isArray(params?.table)
    ? params.table[0]
    : params?.table;
  ...
  const [centuryRateInput, setCenturyRateInput] = useState(
    table?.century_rate ? String(table.century_rate) : ""
  );
  const [centuryRate, setCenturyRate] = useState<number>(
    table?.century_rate ? Number(table.century_rate) : 0
  );

  const [tenRedRateInput, setTenRedRateInput] = useState(
    table?.ten_red_rate ? String(table.ten_red_rate) : ""
  );
  const [tenRedRate, setTenRedRate] = useState<number>(
    table?.ten_red_rate ? Number(table.ten_red_rate) : 0
  );

  const [oneRedRateInput, setOneRedRateInput] = useState(
    table?.one_red_rate ? String(table.one_red_rate) : ""
  );
  const [oneRedRate, setOneRedRate] = useState<number>(
    table?.one_red_rate ? Number(table.one_red_rate) : 0
  );

  const [sixRedRateInput, setSixRedRateInput] = useState(
    table?.six_red_rate ? String(table.six_red_rate) : ""
  );
  const [sixRedRate, setSixRedRate] = useState<number>(
    table?.six_red_rate ? Number(table.six_red_rate) : 0
  );

  const [fifteenRedRateInput, setFifteenRedRateInput] = useState(
    table?.fifteen_red_rate ? String(table.fifteen_red_rate) : ""
  );
  const [fifteenRedRate, setFifteenRedRate] = useState<number>(
    table?.fifteen_red_rate ? Number(table.fifteen_red_rate) : 0
  );
      ...
      <View style={styles.container}>
        ...
        >
          ...
          <Text style={styles.buttonText}>
            {isLoading ? (
              <ActivityIndicator color={"#fefe"} size={"small"} />
            ) : table?._id ? (
              "Edit Table"
            ) : (
              "Add Table"
