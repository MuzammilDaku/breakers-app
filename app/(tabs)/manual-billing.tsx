...
export default function ManualBilling() {
    ...
    const tableNames = tables?.map((item) => ({ label: item?.name, value: item?.name }));
    ...
    useEffect(() => {
        if (billInfo.customer_name && billInfo.game_type.length > 0 && billInfo.game_mode.length > 0 && billInfo.table_names.length > 0 && billInfo.total_bill > 0) {
            setIsDisabled(false);
        }
        else {
            setIsDisabled(true);
    ...
    const handleClick = async () => {
        try {
            ...
            if (isConnected) {
                ...
                setPaidBills(res);
                ToastAndroid.showWithGravity("Bill Saved", ToastAndroid.LONG, ToastAndroid.CENTER);
            }
            else {
                addToQueue({
                    method: "POST",
            ...
            ToastAndroid.showWithGravity(String(error), ToastAndroid.LONG, ToastAndroid.CENTER);
        ...
        <ScrollView style={styles.container}>
            <View style={{ marginHorizontal: 20 }}>
                ...
                <View style={styles.inputContainer}>
                    ...
                    <TextInput placeholder="Enter Player Name" style={styles.input} value={billInfo.customer_name} onChangeText={(e) => { handleChange("customer_name", e) }} />
                </View>
                <View style={styles.inputContainer}>
                ...
                <View style={styles.inputContainer}>
                    ...
                    <TextInput placeholder="Enter Amount" style={styles.input} value={String(billInfo.total_bill)} onChangeText={(e) => { handleChange("total_bill", Number(e)) }} />
                </View>
